...
76～83行目　動的メモリを確保する。新たに確保したデータ領域をpNodeとして、pNodeに市町村のデータを入れる。
            pNodeの要素の部分に先頭の要素の値を代入して、新たなデータ領域とつなげる。
103～120行目　IDが一致する市町村があるかどうか順に上から探し、IDが一致しなければ次の要素をさすようにアドレスを入れ替え
            　一致すればｐCityに一致したIDの市町村のデータを代入する。この動作をwhile文で要素がなくなるまで繰り返し、   
            　なくなった場合ー1を返す。
