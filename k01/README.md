...
## ソースコードの説明
67,68行目　比較対象の位置を"pos"として、結果の値を"result"として引数に定義する。最初はresultに-1を代入しておく。
70～75行目　探しているIDと一致したときの比較対象の位置(posの値)を結果として返す。一致しなければ比較対象の位置を一つ右にずらす。
            posの値がリストの最後の位置より大きければ検索終了となり、resultは-1のまま。
82～85行目　比較対象の位置を"pos"として、結果の値を"result"として、リストの左端を"left"右端を"right"として引数に定義する。
               leftには"0"、rightには"MAX_CITY-1"を代入する。
86～95行目　比較対象の位置をリストの中央に設定して、探す値と比較対象の位置の値を比較し
            探す値の方が大きい場合リストの左端の位置をposの一つ右にずらし、小さい場合は右端の位置をposの一つ左にずらす事によって
            比較対象の範囲を分割して探索できる。一致したらその値(pos)を、比較する要素がなければ-1を結果として返す。
